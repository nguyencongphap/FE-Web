...
const EditNavigateCartBar = ({movieId, quantity}) => {
    ...
    const decreaseItemQuantity = async () => {
        ...
        if (curQuantity.current === 0) {
            await requestDELETECartDelete(accessToken, movieId)
        }
        else {
            await requestPOSTCartUpdate(accessToken, movieId, curQuantity.current)
        }
            ...
            {/*<Button>From AddToCartBar State: {curQuantityToDisplay}</Button>*/}
            {
                curQuantityToDisplay === 0 ?
                    <Button
                        ...
                        size="medium"
                        onClick={async () => {
                            await requestPOSTCartInsert(accessToken, movieId, 1)
                            curQuantity.current = 1
...
export default EditNavigateCartBar;
